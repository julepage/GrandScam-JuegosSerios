export default class EscenaCuestionario extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(centerX, centerY - 250, this.textos.titulo, { fontSize: '32px', color: '#ffffff' }).setOrigin(0.5);
        ...
        this.mensajeError = this.add.text(centerX, startY + gap * 6.05, '', { fontSize: '27px', color: '#22ff00ff' }).setOrigin(0.5);
    ...
    createInputField(x, y, labelText, fieldKey, required = true) {
        this.add.text(x - 180, y, `${labelText}:`, { fontSize: '24px', color: required ? '#ffffff' : '#cccccc', align: 'right' }).setOrigin(1, 0.5);
        ...
        const text = this.add.text(x + 60, y, '', { fontSize: '22px', color: '#ffffff' }).setOrigin(0.5);
